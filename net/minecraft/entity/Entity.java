...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            Resolute.onEvent(safeWalk);

            boolean flag = this.onGround && (this.isSneaking() || safeWalk.isCancelled()) && this instanceof EntityPlayer;
            ...
            EventStep event = new EventStep(stepHeight, this);
            event.setType(EventType.PRE);
            Resolute.onEvent(event);

            if (event.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z))
            {
                double d11 = x;
                double d7 = y;
                double d8 = z;
    ...
    public Vec3d getPositionEyes2(float partialTicks)
    {
        if (partialTicks == 1.0F)
        {
    ...
    {
        if(Hitboxes.enabled) {
            return (float) Hitboxes.size.getValue();
        } else {
            return 0.1F;
        }
    }
