...
import vip.Resolute.events.impl.EventPlayerController;
            ...
            if (!targetEntity.hitByEntity(this))
            {
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    if (flag2)
                    {
                        if (i > 0)
                        {
                            EventPlayerController eventPlayerController = new EventPlayerController(this.rotationYaw, 0.6, false);
                            Resolute.onEvent(eventPlayerController);

                            targetEntity.addVelocity((-MathHelper.sin(eventPlayerController.getYaw() * (float)Math.PI / 180.0F) * (float)i * 0.5F), 0.1D, (MathHelper.cos(eventPlayerController.getYaw() * (float)Math.PI / 180.0F) * (float)i * 0.5F));
                            this.motionX *= eventPlayerController.getMotion();
                            this.motionZ *= eventPlayerController.getMotion();
                            this.setSprinting(eventPlayerController.isSprinting());
