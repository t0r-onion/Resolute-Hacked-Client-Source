...
import vip.Resolute.events.EventType;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public EventMotion currentEvent;
    ...
    {
        EventMotion eventMotion = new EventMotion(this.lastReportedYaw, this.lastReportedPitch, this.posX, this.getEntityBoundingBox().minY, this.posZ, this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ, this.rotationYaw, this.rotationPitch, this.onGround);
        this.currentEvent = eventMotion;
        eventMotion.setType(EventType.PRE);
        Resolute.onEvent(eventMotion);

        EventSprint eventSprint = new EventSprint(this.isSprinting());
        Resolute.onEvent(eventSprint);

        boolean flag = eventSprint.isSprinting();
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            if(eventMotion.isRotating()) {
                //this.rotationYawHead = (float) RenderUtils.interpolate(eventMotion.getPrevYaw(), eventMotion.getYaw(), 0);
                //this.renderYawOffset = (float) RenderUtils.interpolate(eventMotion.getPrevYaw(), eventMotion.getYaw(), 0);

                /*
                if(KillAura.target != null) {
                    RendererLivingEntity.yawHead = (float) RenderUtils.interpolate(eventMotion.getPrevYaw(), eventMotion.getYaw(), 0);
                    RendererLivingEntity.bodyOffsetYaw = (float) RenderUtils.interpolate(eventMotion.getPrevYaw(), eventMotion.getYaw(), 0);
                } else {
                    this.rotationYawHead = (float) RenderUtils.interpolate(eventMotion.getPrevYaw(), eventMotion.getYaw(), 0);
                    this.renderYawOffset = (float) RenderUtils.interpolate(eventMotion.getPrevYaw(), eventMotion.getYaw(), 0);
                }
    ...
    public void displayGUIChest(IInventory chestInventory)
    {
        ...
        if ("minecraft:chest".equals(s))
        {
            ...
            EventOpenChest eventOpenChest = new EventOpenChest(chest);
            Resolute.onEvent(eventOpenChest);
        }
    ...
    public void onLivingUpdate()
    {
        ...
        Resolute.onEvent(eventSlow);

        if (!eventSlow.isCancelled() && this.isUsingItem() && !this.isRiding())
        {
