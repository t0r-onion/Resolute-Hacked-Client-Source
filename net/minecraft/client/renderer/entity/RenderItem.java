    ...
    private void renderEffect(IBakedModel model)
    {
        if (!Config.isCustomItems() || CustomItems.isUseGlint())
        {
            if (!Config.isShaders() || !Shaders.isShadowPass)
            {
                ...
                final int col = Glint.enabled ? Glint.getColor() : -8372020;
        ...
        if (stack != null && entityToRenderFor != null)
        {
            ...
            if (entityToRenderFor instanceof EntityPlayer)
            {
                ...
                else if (item == Items.bow && entityplayer.getItemInUse() != null)
                {
                    ...
                    try {
                        if (entityplayer.getItemInUseCount() > 0 && (entityplayer.getHeldItem().getItemUseAction() == EnumAction.BLOCK) || (KillAura.blocking && KillAura.autoblockModeProp.is("Fake"))) {
                            GL11.glTranslatef(0.05F, entityplayer.isSneaking() ? 0.2F : 0.0F, -0.1F);
