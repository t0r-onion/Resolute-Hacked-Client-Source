...
import vip.Resolute.events.impl.EventRenderWorld;
import vip.Resolute.modules.impl.combat.Reach;
import vip.Resolute.modules.impl.render.Camera;
import vip.Resolute.modules.impl.render.RearView;
import vip.Resolute.modules.impl.render.WorldColor;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.theWorld != null)
        {
            ...
            if(Reach.enabled) {
                reach = (float) Reach.reach.getValue();
            } else {
                reach = 3.0f;
            }
            ...
            Vec3 vec33 = null;
            float f = 1.0F;
            List list = this.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand((double)f, (double)f, (double)f), Predicates.and(EntitySelectors.NOT_SPECTATING, new EntityRenderer_1(this)));
            double d2 = d1;
    ...
    public void orientCamera(float partialTicks)
    {
        ...
        EventCameraPosition event = new EventCameraPosition(entity.getEyeHeight());
        Resolute.onEvent(event);

        float f = (float) (entity.getEyeHeight() - (event.isCancelled() ? event.getY() : 0));
    ...
    private void updateLightmap(float partialTicks)
    {
        if (this.lightmapUpdateNeeded)
        {
            ...
            if (worldclient != null)
            {
                ...
                if(WorldColor.enabled) {
                    final int color = WorldColor.lightMapColorProperty.getColor();
        ...
        if (!this.mc.skipRenderWorld)
        {
            ...
            if (this.mc.currentScreen != null)
            {
                ...
                catch (Throwable throwable)
                {
                    ...
                    crashreportcategory.addCrashSectionCallable("Screen name", new EntityRenderer_2(this));
                ...
                if(Overlay.notif.isEnabled())
                    Resolute.getNotificationManager().render(scaledresolution, null, false, 2);
            }
        }
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        Resolute.onEvent(new EventRenderWorld());
        ...
        Resolute.onEvent(eventRender3D);
    ...
    private void renderWorldPass2(final int pass, final float partialTicks, final long finishTimeNano) {
        ...
        final int size = 6;
        final int width = (mc.displayWidth / size);
        final int height = (mc.displayHeight / size);

        if (flag) {
            Shaders.setViewport(100, 100, width, height);
        } else {
            GlStateManager.viewport(8, 5, width, height);
        }
        ...
        this.mc.mcProfiler.endStartSection("camera");
        this.setupCameraTransform2(partialTicks, pass);
        ...
        this.mc.mcProfiler.endStartSection("hand");
        final boolean flag2 = ReflectorForge.renderFirstPersonHand(this.mc.renderGlobal, partialTicks, pass);
    ...
    public void setupCameraTransform2(final float partialTicks, final int pass) {
        ...
        if (this.cameraZoom != 1.0D) {
            GlStateManager.translate(this.cameraYaw, (-this.cameraPitch), 0.0F);
            GlStateManager.scale(this.cameraZoom, this.cameraZoom, 1.0D);
        }
        ...
        this.orientCamera2(partialTicks);
    ...
    public void orientCamera2(final float partialTicks) {
        final Entity entity = this.mc.getRenderViewEntity();
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
            ...
            if (!this.mc.gameSettings.debugCamEnable) {
                ...
                if (Reflector.ForgeHooksClient_orientBedCamera.exists()) {
                    ...
                    final int j = ((EnumFacing) iblockstate.getValue(BlockBed.FACING)).getHorizontalIndex();
                    GlStateManager.rotate((float) (j * 90), 0.0F, 1.0F, 0.0F);
                }
        ...
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
            if (!this.mc.gameSettings.debugCamEnable) {
                ...
                final Object object = Reflector.newInstance(Reflector.EntityViewRenderEvent_CameraSetup_Constructor, new Object[] { this, entity, block1, Float.valueOf(partialTicks), Float.valueOf(f6), Float.valueOf(f7), Float.valueOf(f8) });
                Reflector.postForgeBusEvent(object);
                f8 = Reflector.getFieldValueFloat(object, Reflector.EntityViewRenderEvent_CameraSetup_roll, f8);
                f7 = Reflector.getFieldValueFloat(object, Reflector.EntityViewRenderEvent_CameraSetup_pitch, f7);
                f6 = Reflector.getFieldValueFloat(object, Reflector.EntityViewRenderEvent_CameraSetup_yaw, f6);
