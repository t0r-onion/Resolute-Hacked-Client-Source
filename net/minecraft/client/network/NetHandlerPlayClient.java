...
import vip.Resolute.events.impl.EventEntitySwing;
import com.google.common.collect.Maps;
import com.google.common.util.concurrent.FutureCallback;
import com.google.common.util.concurrent.Futures;
import com.mojang.authlib.GameProfile;
import vip.Resolute.Resolute;
import vip.Resolute.events.impl.EventParticleSpawn;
    ...
    public void handleEntityMetadata(S1CPacketEntityMetadata packetIn)
    {
        ...
        if (entity != null && packetIn.func_149376_c() != null)
        {
            if (entity instanceof EntityLivingBase && !(entity instanceof EntityPlayerSP)) {
                final DataWatcher.WatchableObject watchableObject = watchableObjects.get(0);
                if (watchableObject.getDataValueId() == 6 && watchableObject.getObjectType() == 3) {
                    final float newHealth = (float)watchableObject.getObject();
                    final EntityLivingBase elb = (EntityLivingBase)entity;
                    final float health = elb.getHealth();
                    final float dif = health - newHealth;
                    if (dif >= 0.0f && newHealth != 10.0f) {
                        EventEntityDamage event = new EventEntityDamage(elb, dif);
    ...
    public void addToSendQueue(Packet p_147297_1_)
    {
        ...
        if(p_147297_1_ instanceof C03PacketPlayer.C05PacketPlayerLook) {
            ...
            /*
                ...
                Minecraft.getMinecraft().thePlayer.renderYawOffset = packet.getYaw();

             */


            /*
            RendererLivingEntity.yawHead = packet.getYaw();
            RendererLivingEntity.prevYawHead = packet.getYaw();
 ...
 */
    ...
    public void handleAnimation(S0BPacketAnimation packetIn)
    {
        ...
        if (entity != null)
        {
            if (packetIn.getAnimationType() == 0)
            {
                ...
                EventEntitySwing event = new EventEntitySwing(entitylivingbase);
                Resolute.onEvent(event);
    ...
    private void spawnParticleEntityEvent(final Entity entity, final int type) {
        final EnumParticleTypes particleType = (type == 0) ? EnumParticleTypes.CRIT : EnumParticleTypes.CRIT_MAGIC;
        int multiplier;
        if (entity instanceof EntityPlayerSP) {
            multiplier = 1;
        }
        else {
            final EventParticleSpawn event = new EventParticleSpawn(type, 1);
            Resolute.onEvent(event);
        ...
        switch (multiplier) {
            case 0: {}
            case 1: {
                this.gameController.effectRenderer.emitParticleAtEntity(entity, particleType);
                break;
            }
            default: {
