...
import vip.Resolute.events.impl.Event2D;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        LockedResolution lockedResolution = RenderUtils.getLockedResolution();
        ...
        if (scoreobjective1 != null)
        {
            ...
            float scoreboardX = (float) Overlay.scoreboardX.getValue();
            float scoreboardY = (float) Overlay.scoreboardY.getValue();
            int offset = (int) ((scaledresolution.getScaledHeight()/3) - scoreboardY);

            if(Overlay.scoreboard.isEnabled()) {
        ...
        if(Overlay.notif.isEnabled())
            Resolute.getNotificationManager().render(null, lockedResolution, true, 2 + potionY);

        EventRender2D eventRender2D = new EventRender2D(partialTicks);
        Resolute.onEvent(eventRender2D);
    ...
    {
        EventCrosshair event = new EventCrosshair();
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_)
    {
        ...
        for (Object score10 : arraylist1)
        {
            ...
            if(k == 1) {
                RenderUtils.drawWaveString("www.resolute.pub", j, l);
            } else {
