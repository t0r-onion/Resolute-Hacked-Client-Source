...
public class GuiTextField extends Gui
{
    ...
    public GuiTextField(int x, int y, String text, String name)
    {
        this.xPosition = x;
        this.yPosition = y;
        this.name = name;
    ...
    public void mouseClicked(int mouseX, int mouseY){
        if(mouseX > xPosition && mouseX < (length > 50 ? xPosition + length : xPosition + FontUtil.moon.getStringWidth(name)) && mouseY > yPosition && mouseY < yPosition +  FontUtil.moon.getHeight()){
            this.isFocused = true;
        }else {
    ...
    public void draw(){
        ...
        float diff = (float) (FontUtil.moon.getStringWidth(text) - length);
        length += diff / 4;
        if(FontUtil.moon.getStringWidth(text) < 50){
            Gui.drawRect(xPosition, yPosition + FontUtil.moon.getHeight() + 3, xPosition + 50 + FontUtil.moon.getStringWidth(name) - 40, yPosition + FontUtil.moon.getHeight() + 4, -1);
        }else {
            Gui.drawRect(xPosition, yPosition + FontUtil.moon.getHeight() + 3, xPosition + length + 7, yPosition + FontUtil.moon.getHeight() + 4, -1);
        }
        if(text.equalsIgnoreCase("") && !isFocused){
            FontUtil.moon.drawString(name, xPosition, yPosition + 1, new Color(160, 160, 160, 255).getRGB());
        }
        if(!timer.hasTimeElapsed(1000, true)) {
            if(timer.hasTimeElapsed(500, true) && isFocused) {
                FontUtil.moon.drawString(text + "|", xPosition, yPosition + 1, new Color(160, 160, 160, 255).getRGB());
                // Artemis.instance.cfrs.drawString(text, xPosition, yPosition + 1, new Color(160, 160, 160, 255).getRGB());
            }else {
                FontUtil.moon.drawString(text, xPosition, yPosition + 1, new Color(160, 160, 160, 255).getRGB());
            }
        }

        if(timer.hasTimeElapsed(950, true)){
            timer.reset();
        }
        GlStateManager.color(1.0f, 1.0f,1.0f,1.0f);
