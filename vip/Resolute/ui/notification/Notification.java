...
public final class Notification {
    ...
    public Notification(final String title, final String body, final long duration, final NotificationType type) {
        ...
        this.body = body;
        if (body != null) {
            this.width = (float) (Math.max(fontRenderer.getStringWidth(title), fontRenderer.getStringWidth(body)) + 4.0f);
        }
        else {
            this.width = (float) (fontRenderer.getStringWidth(title) + 4.0f);
        }
        this.height = 27.0f;
    ...
    public void render(final LockedResolution lockedResolution, final ScaledResolution scaledResolution, final int index, final int yOffset) {
        ...
        Gui.drawRect(x, y, x + this.width, y + this.height, 0x7F1f211f);
