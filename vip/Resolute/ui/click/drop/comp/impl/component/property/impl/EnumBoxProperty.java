...
public final class EnumBoxProperty extends ExpandableComponent implements PropertyComponent
{
    ...
    public void drawComponent(final ScaledResolution scaledResolution, final int mouseX, final int mouseY) {
        ...
        Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(this.getName(), (float)(x + 2), y + this.getHeight() / 2.0f - 3.0f, textColor);
    ...
    private void handleRender(final int x, int y, final int width) {
        ...
        for (final String e : property.getModes()) {
            ...
            Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(StringUtils.upperSnakeCaseToPascal(e.toString()), (float)(x + 1 + 2), (float)y, property.is(e) ? ((int)ClickGUI.color.getColor()) : new Color(255, 255, 255).getRGB());
            y += 12;
        }
    }
