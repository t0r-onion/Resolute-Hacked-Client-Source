...
public class Nametags extends Module {
        ...
        if(e instanceof EventRender3D) {
            ...
            while(var3.hasNext()) {
                ...
                if (!player.isInvisible() && !(player instanceof EntityPlayerSP)) {
                    double x = player.lastTickPosX + (player.posX - player.lastTickPosX) * (double)er.getPartialTicks() - RenderManager.renderPosX;
                    double y = player.lastTickPosY + (player.posY - player.lastTickPosY) * (double)er.getPartialTicks() - RenderManager.renderPosY;
                    double z = player.lastTickPosZ + (player.posZ - player.lastTickPosZ) * (double)er.getPartialTicks() - RenderManager.renderPosZ;
    ...
    public void renderNametag(EntityPlayer player, double x, double y, double z) {
        ...
        RenderUtils.rectangle((double)(-width - 2), (double)(-(mc.fontRendererObj.FONT_HEIGHT - 6)), (double)(width + 2), (double)(mc.fontRendererObj.FONT_HEIGHT + 1), -1728052224);
        GlStateManager.enableTexture2D();
        int color = -1;
        ...
        Color customColor = health >= 0.0F ? RenderUtils.blendColors(fractions, colors, progress).brighter() : Color.RED;
