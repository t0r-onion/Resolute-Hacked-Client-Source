...
public class placeholder {
    ...
    public static NumberSetting oldncpblinkticks = new NumberSetting("OldNCP Blink Ticks", 10, 1, 40, 1);
        ...
        if(mode.equalsIgnorecase("OldNCP Custom")) {
            ...
            if(mc.thePlayer.isMoving()) {
                ...
                if(mc.thePlayer.onGround) {
                    ...
                    if(ticksElapsed < oldncpspeed.getValue()-0.675) {
                        ticksElapsed += oldncpfricition.getValueFloat();
                    }
                    if(oldncptimerboost.isEnabled()) {
                        mc.timer.timerSpeed = oldncptimer.getValueFloat() + 2;
                    }
                    mc.thePlayer.motionY = 0;
                    mc.thePlayer.setPosition(mc.thePlayer.posX, mc.thePlayer.posY - 10, mc.thePlayer.posZ);

                    if(oldncpboost.isEnabled()) {
                        mc.thePlayer.setSpeed(oldncpspeed.getValue() - ticksElapsed);
                    } else {
